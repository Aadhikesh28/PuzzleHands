...
const DIFFICULTIES: Record<string, DifficultyConfig> = {
  ...
  MODERATE: { id: 'moderate', label: 'Moderate', grid: 4 },
...
const floatingReactionIdRef = useRef(0);
  ...
  const correctPieces = pieces.filter(
    p => p.currentIdx === p.originalIdx
  ).length;

  const totalPieces = pieces.length;
  const progress = correctPieces / totalPieces;

  if (progress >= 0.8) {
    setMascotEmoji('🥳');
    setMascotMessage('Almost there!');
  } else if (moves === 0) {
    setMascotEmoji('😀');
    setMascotMessage("Let's solve this!");
  } else if (moves < 3) {
    setMascotEmoji('🤔');
    setMascotMessage('Think carefully...');
  } else if (moves < 8) {
    setMascotEmoji('😮');
    setMascotMessage("You're getting close!");
  } else {
    setMascotEmoji('🔥');
    setMascotMessage('Keep going!');
  }
}, [moves, pieces, phase]);
...
useEffect(() => {
  ...
  0,
  Math.round(baseScore - timePenalty - hintPenalty - movePenalty)
);
...
setMascotEmoji('🎉');
setMascotMessage('Puzzle Solved!');
playVictorySound();
  ...
  const onResults = useCallback((results: any) => {
    ...
    if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
      ...
      // Direct hand-to-screen mapping.
// The video is mirrored, so reverse X exactly once.
const clampedHandX = Math.max(0, Math.min(1, handX));
const clampedHandY = Math.max(0, Math.min(1, handY));
...
if (rect) {
  ...
  y = clampedHandY * window.innerHeight;
      ...
      if (currentPhase === 'solve') {
        ...
        // Calculate the puzzle slot directly from the visible cursor.
// X has already been mirrored above, so DO NOT mirror it again.
let hoveredSlotIdx: number | null = null;

if (rect) {
  const gridX = Math.max(0, Math.min(rect.width, finalX - rect.left));
  const gridY = Math.max(0, Math.min(rect.height, finalY - rect.top));

  const normalizedX = gridX / rect.width;
  const normalizedY = gridY / rect.height;

  const col = Math.floor(normalizedX * gridDimension);
  const row = Math.floor(normalizedY * gridDimension);

  const clampedCol = Math.max(0, Math.min(gridDimension - 1, col));
  const clampedRow = Math.max(0, Math.min(gridDimension - 1, row));

  hoveredSlotIdx = clampedRow * gridDimension + clampedCol;
}

// Cache the last hovered slot index
if (hoveredSlotIdx !== null) {
  lastHoveredSlotIdxRef.current = hoveredSlotIdx;
}

        // Transition: Not pinching -> Pinching (PICKUP)
        if (isPinching && !wasPinching && !curDragged && hoveredSlotIdx !== null) {
        ...
        else if (!isPinching && wasPinching && curDragged && hoveredSlotIdx !== null) {
  ...
  // Find pieces that are currently in the wrong position
  const misplacedPieces = pieces.filter(
    (piece) => piece.currentIdx !== piece.originalIdx
  );

  // If everything is already correct, no hint is needed
  if (misplacedPieces.length === 0) return;

  // Pick one wrong piece randomly
  const randomPiece =
    misplacedPieces[Math.floor(Math.random() * misplacedPieces.length)];
  ...
  <div className="flex flex-col items-center gap-2">
    <div className="flex items-center gap-2 bg-[#111]/80 backdrop-blur-md px-4 py-2 rounded-full border border-white/10">
      <Clock size={16} className="text-[#D7FF2F]" />
    ...
    <button
      onClick={handleHint}
      className="pointer-events-auto px-4 py-2 rounded-full bg-[#111]/80 backdrop-blur-md border border-[#D7FF2F]/40 text-[#D7FF2F] font-bold text-xs tracking-wider hover:bg-[#D7FF2F]/10 transition-all"
    >
      💡 HINT <span className="ml-1 text-zinc-400">({hintsUsed})</span>
    </button>
  </div>
)}
  ...
  <div className="mt-2 px-4 py-2 rounded-full bg-[#111]/80 border border-white/10 text-sm text-white font-medium text-center">
    {mascotMessage}
  </div>
</div>
                ...
                {pieces.map((piece) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`absolute overflow-hidden group cursor-pointer border ${
  isDragged
    ? 'border-[#D7FF2F] z-20 shadow-[0_0_20px_rgba(215,255,47,0.4)]'
    : isHinted
      ? 'border-yellow-400 z-20 shadow-[0_0_25px_rgba(250,204,21,0.7)] ring-2 ring-yellow-400 animate-pulse'
...
<div className="flex flex-col items-center">
  ...
  <span className="text-xl font-bold text-[#D7FF2F] mt-0.5">
    {score}
  </span>
</div>
...
<div className="flex flex-col">
  ...
  <span className="text-xl font-black text-[#D7FF2F] mt-0.5">
    {score}
  </span>
</div>
              </div>
